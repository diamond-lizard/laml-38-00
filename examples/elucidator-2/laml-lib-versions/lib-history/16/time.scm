...
;          ((and (= wn 53) (<= (weekday-number sc) 3)) 1)  ; not OK yet. We live with week 53 and some errors here
          (else wn))
))
...
(define (when-generated)
 (let* ((dt (date-time (current-time)))
  ...
  (con  init-text date ", " time)))


; ---------------------------------------------------------------------------------------------------------------
;;; Time interval functions.

;; Return the number of years, months, weeks, days, hours, minutes, and seconds
...
; ---------------------------------------------------------------------------------------------------------------
;;; Underlying time related functions.
...
; (define time-zone-info -1) ; vintertid - Winter time - DK.

...
; Copyright (C) 1999-2008  Kurt Normark, normark@cs.auc.dk.
; 
; Execution of LAML files from Emacs.
; Top level functions: laml-process-current-buffer, laml-process-buffer,
; and laml-sync-process-current-buffer, and laml-process-file, and laml-sync-process-file.
; In addition, activation of LAML via dired is supported by some additional functions.
; 
; The first section of this file contains two Scheme system dependent, and platform and operating system
; dependent parts: start-laml-process and call-laml-process. You are supposed to augment and modify
; the functions start-laml-process and call-laml-process if you intend to port LAML.
...
(defun start-laml-process (dir file-without-extension extension output-buffer &rest program-parameters)
 "Start an asynchronous LAML process on the file characterized by the first three parameters. Show output in output-buffer.
Possible remaining parameters are passed as LAML program parameters (as of version 33 only in MzScheme). Must all be strings!
...
distinguish between the concrete process startup parameters."
 (let* ((file-with-extension (concat file-without-extension "." extension))  
  ...
  (cond ((or (eq scheme-system 'mzscheme) (eq scheme-system 'mzscheme-200) (eq scheme-system 'mzscheme-300))
           (apply (function start-process) (append (list "LAML" output-buffer scheme-exec  "-v"  "-f" laml-init "-d" file-path file-with-extension dir) program-parameters)))
...
(defun call-laml-process (dir file-without-extension extension output-buffer &rest program-parameters)
...
distinguish between the concrete process startup parameters."
 (let* ((file-with-extension (concat file-without-extension "." extension))
  ...
  (cond ((or (eq scheme-system 'mzscheme) (eq scheme-system 'mzscheme-200) (eq scheme-system 'mzscheme-300))
           (apply (function call-process) (append (list scheme-exec nil output-buffer nil  "-v"  "-f" laml-init "-d" file-path file-with-extension dir) program-parameters)))
...
(defun laml-process-current-buffer (&rest program-parameters)
  ...
  (let* ((cur-dir (current-directory))
    ...
    (apply (function start-laml-process) (append (list cur-dir (file-name-proper buf-name) (file-name-extension buf-name) out-buf) program-parameters))
...
(defun laml-process-buffer (buffer &rest program-parameters)
  ...
  (let* ((cur-dir (current-directory))
    ...
    (erase-buffer)
    (insert (concat "LAML Emacs Processing with " (symbol-to-string scheme-system))) (insert CR)
    (apply (function start-laml-process) (append (list buf-dir filename-without-extension (file-name-extension buf-name) out-buf) program-parameters))))


(defun laml-sync-process-current-buffer (&rest program-parameters)
  ...
  (let* ((cur-dir (current-directory))
    ...
    (apply (function call-laml-process) (append (list cur-dir filename-without-extension (file-name-extension buf-name) laml-output) program-parameters))
...
(defun laml-process-file (file &optional processing-mode &rest program-parameters)
...
can be suppressed by passing the symbol silent as processing mode."
  (let* ((path (file-name-directory file))
    ...
    (apply (function start-laml-process) (append (list path filename-without-extension file-extension out-buf) program-parameters))))

(defun laml-sync-process-file (file &optional processing-mode &rest program-parameters)
...
can be suppressed by passing the symbol silent as processing mode."
  (let* ((path (file-name-directory file))
    ...
    (apply (function call-laml-process) (append (list path filename-without-extension file-extension out-buf) program-parameters))))

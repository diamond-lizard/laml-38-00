...
(laml-define-key laml-mode-map (list "\C-c\C-t" "\C-c\C-t")  'laml-toggle-laml-silent-processing)
...
; String explaining the keybinding of SchemeDoc examples.
(defvar schemedoc-how-to-get-examples "")
...
(defun laml-enable-scheme-mode ()
  ...
  (laml-define-key scheme-mode-map [menu-bar schemedoc]
    (cons "SchemeDoc" (make-sparse-keymap "SchemeDoc")))

  (laml-define-key scheme-mode-map [menu-bar schemedoc schemedoc-help]
      '("SchemeDoc Help" . schemedoc-help))

  (laml-define-key scheme-mode-map [menu-bar schemedoc menu-scheme-forms-sep-2]
    '("----"))


  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-misc-clause]
      '("Insert miscellaneous clause" . (lambda () (interactive (laml-insert-template "schemedoc-misc-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-comment-clause]
      '("Insert internal comment clause" . (lambda () (interactive (laml-insert-template "schemedoc-comment-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-post-condition-clause]
      '("Insert postcondition clause" . (lambda () (interactive (laml-insert-template "schemedoc-post-condition-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-returns-clause]
      '("Insert returns clause" . (lambda () (interactive (laml-insert-template "schemedoc-returns-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-reference-clause]
      '("Insert external reference clause" . (lambda () (interactive (laml-insert-template "schemedoc-reference-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-internal-references-clause]
      '("Insert internal references clause" . (lambda () (interactive (laml-insert-template "schemedoc-internal-references-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-example-clause]
      '("Insert example clause" . (lambda () (interactive (laml-insert-template "schemedoc-example-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-attribute-clause]
      '("Insert attribute clause" . (lambda () (interactive (laml-insert-template "schemedoc-attribute-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-parameter-templates]
      '("Insert parameter templates" . insert-parameter-templates))
  
  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-parameter-clause]
      '("Insert parameter clause" . (lambda () (interactive (laml-insert-template "schemedoc-parameter-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-pre-condition-clause]
      '("Insert precondition clause" . (lambda () (interactive (laml-insert-template "schemedoc-pre-condition-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-form-clause]
      '("Insert form clause" . (lambda () (interactive (laml-insert-template "schemedoc-form-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-dependencies-clause]
      '("Insert schemedoc dependencies clause" . (lambda () (interactive (laml-insert-template "schemedoc-dependencies-clause")))))



  (laml-define-key scheme-mode-map [menu-bar schemedoc menu-scheme-forms-sep-4]
    '("----"))


  (laml-define-key scheme-mode-map [menu-bar schemedoc definition-comment]
      '("Insert definition comment" . schemedoc-insert-definition-comment))

  (laml-define-key scheme-mode-map [menu-bar schemedoc section-comment]
      '("Insert section comment" . schemedoc-insert-section-comment))

  (laml-define-key scheme-mode-map [menu-bar schemedoc introduction-comment]
      '("Insert abstract comment" . schemedoc-insert-introduction-comment))



  (laml-define-key scheme-mode-map [menu-bar schemedoc menu-scheme-forms-sep-3]
    '("----"))

  (laml-define-key scheme-mode-map [menu-bar schemedoc insert-source-dependencies-clause]
      '("Insert source file dependencies clause" . (lambda () (interactive (laml-insert-template "sourcefile-dependencies-clause")))))

  (laml-define-key scheme-mode-map [menu-bar schemedoc reset-scheme-completion-from-source-file]
      '("Reset source file(s) completion support" . reset-scheme-completion-from-source-file))

  (laml-define-key scheme-mode-map [menu-bar schemedoc reset-scheme-completion]
      '("Reset SchemeDoc AND source file(s) completion support" . reset-scheme-completion))

  (laml-define-key scheme-mode-map [menu-bar schemedoc reset-schemedoc-information]
      '("Reset SchemeDoc support" . reset-schemedoc-information))

  (laml-define-key scheme-mode-map [menu-bar schemedoc info-emacs-schemedoc]
      '("Info about Emacs SchemeDoc support" . info-emacs-schemedoc))

  (laml-define-key scheme-mode-map [menu-bar schemedoc display-schemedoc-information]
      '("Show documentation of..." . display-schemedoc-information))


  (laml-define-key scheme-mode-map [menu-bar schemedoc menu-scheme-forms-sep-1]
    '("----"))

  (laml-define-key scheme-mode-map [menu-bar schemedoc run-schemedoc]
...
;  (laml-define-key scheme-mode-map "\C-c\C-a" 'schemedoc-insert-introduction-comment)
  ...
  (laml-define-key scheme-mode-map (list "\C-hf" "\C-hf") 'display-schemedoc-information)  ; Scheme
  (laml-define-key scheme-mode-map (list "\C-hg" "\C-hg") 'describe-function)              ; E-lisp
  (laml-define-key scheme-mode-map (list "\C-i" "\C-i") 'scheme-complete-symbol)  ; TAB
  (laml-define-key scheme-mode-map (list (kbd "<backtab>") (kbd "<backtab>")) 'scheme-give-examples)  ; Shift TAB
  (laml-define-key scheme-mode-map (list (kbd "½") (kbd "½")) 'scheme-complete-symbol-local-only)
  (setq schemedoc-how-to-get-examples "Use Shift-TAB.")

  (laml-define-key scheme-mode-map "\C-c\C-r" 'reset-scheme-completion) 
  (laml-define-key scheme-mode-map "\C-cr" 'reset-scheme-completion-from-source-file)
  (laml-define-key scheme-mode-map "\C-cR" 'reset-schemedoc-information)

  (laml-define-key scheme-mode-map "\C-c\C-p" 'insert-parameter-templates)
  ...
  (if (fboundp 'find-lisp-definition-of) 
    (progn
      ; Scheme Function keys - navigation from applied to define name occurrences:
      (laml-define-key scheme-mode-map [f5] 'pop-lisp-file-position)
      (laml-define-key scheme-mode-map [f6] 'pop-ghost-lisp-file-position)
      (laml-define-key scheme-mode-map [f7] 'find-lisp-definition-of)
      (laml-define-key scheme-mode-map [S-f7] 'find-lisp-definition-of-and-reset)

      (define-key scheme-mode-map [S-f8] 'initiate-multi-file-name-search)
      (define-key scheme-mode-map [f8] 'continue-multi-file-name-search)
      (define-key scheme-mode-map [C-f8] 'reset-multi-file-name-search)

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find]
                       (cons "Find Scheme definition" (make-sparse-keymap "Find Scheme Definition")))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find help-with-source-file-finding]
                       '("Help" . help-with-source-file-finding))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find info-about-source-file-for-finding-definitions]
                       '("Info about source file finding" . info-about-source-file-for-finding-definitions))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find reset-source-file-finding]
                       '("Reset Scheme Name Finding" . reset-source-file-finding))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find sep-1]
                       '("----"))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find remove-current-source-file-for-finding-definitions]
                       '("Remove current source file" . remove-current-source-file-for-finding-definitions))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find add-current-source-file-for-finding-definitions]
                       '("Add current source file" . add-current-source-file-for-finding-definitions))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find sep-2]
                       '("----"))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find pop-ghost-lisp-file-position]
                       '("Go forward" . pop-ghost-lisp-file-position))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find pop-lisp-file-position]
                       '("Go back" . pop-lisp-file-position))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find find-lisp-definition-of-and-reset]
                       '("Find Scheme Definition and Reset" . find-lisp-definition-of-and-reset))

      (laml-define-key scheme-mode-map [menu-bar scheme scheme-source-find find-lisp-definition-of]
                       '("Find Scheme Definition" . find-lisp-definition-of))
...
(let ((laml-kind (laml-kind-of-distribution)))
  (if (not (eq laml-kind 'slim))
      (setq scheme-mode-hook (list (function laml-enable-scheme-mode)))))
...
(laml-define-key laml-mode-map [menu-bar laml laml-toggle-laml-silent-processing]
      '("Toggle Silent LAML Processing" . laml-toggle-laml-silent-processing))
